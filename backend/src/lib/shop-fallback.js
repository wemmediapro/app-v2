...
module.exports = {
  ...
  getPromotions() {
    ...
    return promotions.map(p => ({
      ...p,
      id: p._id?.toString(),
      validFrom: p.validFrom && (typeof p.validFrom === 'string' ? p.validFrom : p.validFrom.toISOString?.()?.slice(0, 10)),
      validUntil: p.validUntil && (typeof p.validUntil === 'string' ? p.validUntil : p.validUntil.toISOString?.()?.slice(0, 10)),
    }));
  },
};
