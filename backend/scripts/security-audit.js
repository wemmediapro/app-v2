...
function walkJsFiles(dir, out) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      if (skipDirs.has(e.name) || e.name === '__tests__') {
        continue;
      }
...
if (fs.existsSync(validateInputPath)) {
  ...
  if (all) {
    PASSED.push('✅ validateInput.js: exports attendus présents');
  }
} else {
...
if (fs.existsSync(validationPath)) {
  ...
  if (ok) {
    PASSED.push('✅ validation.js: symboles clés présents');
  }
}
