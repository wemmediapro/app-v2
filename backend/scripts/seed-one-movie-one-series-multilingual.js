...
- NE fournis AUCUNE URL (pas de bande-annonce, pas de vidéo).`;
  ...
  const userPrompt =
    'Génère 1 film et 1 série au format JSON : { "items": [ { type: "movie", ... }, { type: "series", ... } ] }. Réponse : uniquement le JSON, pas de ```json.';
  ...
  if (!list || list.length < 2) {
    throw new Error('Il faut au moins 1 film et 1 série dans la réponse');
  }
  return [
...
async function run() {
  ...
  try {
    ...
    console.log(
      '\n✅ Terminé. 1 film + 1 série ajoutés (multilingue, affiches générées, sans URL vidéo). Total actifs:',
      total
    );
  } catch (err) {
