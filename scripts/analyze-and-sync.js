 ...
 * Usage: node scripts/analyze-and-sync.js [--sync-db] [--dry-run]
 */
...
function runSyncDb() {
  ...
  if (missing.length === 0) {
    console.log('Aucun modèle manquant. Base cohérente.');
    return;
  }
  console.log('Modèles déjà créés par le script (fichiers .js). Pour créer les collections en base, démarrer le serveur backend avec MongoDB.');
  console.log('Modèles requis présents:', existing.join(', '));
...
if (missingModels.length && !dryRun) {
  console.log('\n💡 Les modèles Movie et RadioStation ont été ajoutés. Redémarrez le backend pour les utiliser.');
}
