            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 md:gap-5 max-w-full">
              {HOME_SERVICES.map((service, index) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-br ${service.glow} pointer-events-none`}
                      aria-hidden
                    />
                    <div
                      className={`relative flex items-center justify-center flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl ${service.bgLight} border border-white/80 shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all duration-200`}
                    >
            ...
            {homePromosCombined.map((promo, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-widest text-white/90 truncate flex-1 min-w-0 text-right">
                    ...
                    <div className="mt-3 flex items-center gap-2 flex-wrap">
                      ...
                      {hasDiscount && (
                        <span className="text-xs font-bold bg-white/30 backdrop-blur px-2.5 py-1 rounded-full">
                          -{promo.discount}%
                        </span>
                      )}
                    </div>
              ...
              const promoDesc =
                promo.translations && promo.translations[language] && promo.translations[language].description
                  ...
                  : promo.description || '';
              const discountLabel =
                promo.discountType === 'percentage' ? `-${promo.discountValue || 0}%` : `-${promo.discountValue || 0}€`;
              return (
                <motion.button
                ...
                >
                  ...
                  <p className="text-sm font-bold uppercase tracking-wide text-white/90 line-clamp-1 mt-1">
                    {promoTitle}
                  </p>
                  {promoDesc ? (
                    <p className="text-xs text-white/90 mt-2 line-clamp-2 leading-relaxed flex-1 min-h-0">
                      {promoDesc}
                    </p>
                  ) : (
                  ...
                  <div className="mt-3">
                    <span className="inline-flex text-sm font-bold bg-white/25 backdrop-blur px-2.5 py-1 rounded-full">
                      {discountLabel}
                    </span>
                  </div>
                  <p className="text-xs text-white/90 mt-auto pt-4 flex items-center gap-1 font-medium">
      ...
      <section className="hidden md:block px-3 sm:px-4 md:px-5 mt-4 md:mt-6 pb-8 md:pb-12 max-w-4xl mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {QUICK_ACCESS.map(({ key, icon: Icon, labelKey, bg, border, text }) => (
            ...
            >
              <span
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${bg} border border-white/80 shadow-sm`}
              >
