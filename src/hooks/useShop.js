...
export function useShop(language, t) {
  ...
  const filteredShopProducts = useMemo(() => shopProducts.filter(product => {
    ...
    const matchesSearch = shopSearchQuery === '' ||
      ...
      (product.features && Array.isArray(product.features) && product.features.some(f => String(f).toLowerCase().includes(shopSearchQuery.toLowerCase())));
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const hint = error?.response?.status === 503
            ...
            : (error?.response?.status === 404 || error?.code === 'ERR_NETWORK' ? 'Démarrez le backend (port 3000).' : '');
