...
const isDashboardTarget =
  ...
  (process.env.PLAYWRIGHT_BASE_URL && process.env.PLAYWRIGHT_BASE_URL.includes('5174'));
...
test.describe('Dashboard admin', () => {
  ...
  test.beforeEach(() => {
    test.skip(
      !isDashboardTarget,
      'Dashboard tests require PLAYWRIGHT_DASHBOARD_URL or PLAYWRIGHT_BASE_URL=...5174 (dashboard server on 5174)'
    );
  });

  test('sans auth, redirection vers login ou formulaire visible', async ({ page }) => {
    await page.goto('/dashboard/');
    await expect(page.getByRole('heading', { name: /connexion|login|sign in|gnv dashboard/i }).first()).toBeVisible({
      timeout: 10000,
    });
  });
